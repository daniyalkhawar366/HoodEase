...
async function seedDatabase() {
  try {
    ...
    // Keep admin user
    await User.deleteMany({ email: { $ne: 'Admin@Hoodease.com' } });
    ...
    const allProducts = [
      ...
      ...productsWomen.map(p => ({ ...p, id: undefined })),
      ...productsKids.map(p => ({ ...p, id: undefined }))
    ];
    ...
    for (const userData of usersToCreate) {
        ...
        if (!userExists) {
            const user = new User({ ...userData, dateOfBirth: new Date(userData.dateOfBirth) });
            await user.save();
        }
    }
    const createdUsers = await User.find({ isAdmin: { $ne: true } });
    console.log(`Created or found ${createdUsers.length} test users`);

    // --- Admin User ---
    const adminExists = await User.findOne({ email: 'Admin@Hoodease.com' });
    if (!adminExists) {
    ...
    const dbUsers = await User.find({ isAdmin: { $ne: true } });
